window['simple-puzzle'] = {
    ...
    modal.innerHTML = `
      <div class="game-modal-content" style="position:relative; max-height:100vh; overflow-y:auto; box-sizing:border-box; padding:16px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
        <div class="game-modal-header">
          <h2 style="color: white; text-shadow: 2px 2px 4px rgba(0,0,0,0.5);">🧩 פאזל פשוט - 9 חלקים</h2>
        </div>
        <div class="game-modal-body" style="display: flex; flex-direction: column; align-items: center;">
          ...
          <div id="puzzle-instructions" style="color: white; font-size: 1.1rem; margin-bottom: 16px; text-align: center; background: rgba(255,255,255,0.1); padding: 12px; border-radius: 12px;">
            🎯 גרור כל חלק למקום הנכון בלוח!<br>
    ...
    if (progress) {
      ...
      progress.innerHTML = `
        <div style="color: white; font-size: 1.2rem; font-weight: bold; margin-bottom: 8px; text-align: center;">
          🧩 פאזל ${this.stage + 1} - ${this.correctPieces}/9 חלקים
        </div>
  ...
  createPuzzle() {
    ...
    boardContainer.style.top = '30px';
    boardContainer.style.left = '30px';
    ...
    piecesContainer.style.top = '30px';
    piecesContainer.style.right = '30px';
    ...
    piecesArea.style.width = '280px';
    piecesArea.style.height = '320px';
  ...
  createPuzzlePieces(piecesArea) {
    ...
    for (let row = 0; row < this.gridSize; row++) {
      for (let col = 0; col < this.gridSize; col++) {
        ...
        const randomX = Math.random() * (260 - this.pieceSize);
        const randomY = Math.random() * (300 - this.pieceSize);
    ...
    this.showFeedback('🎯 גרור כל חלק למקום הנכון!', '#ffeb3b');
  },
  ...
  createPuzzlePiece(index, row, col, size) {
    ...
    piece.style.boxShadow = '0 0 0 1px rgba(0,0,0,0.2)';
  ...
  connectPiece(piece, correctX, correctY) {
    ...
    setTimeout(() => piece.classList.remove('snapping'), 300);
    ...
    this.correctPieces++;
    this.showFeedback(`🎉 מצוין! ${this.correctPieces}/9 חלקים`, '#4caf50');
  ...
  shufflePieces() {
    this.pieces.forEach(piece => {
      if (!piece.connected) {
        ...
        const randomX = (piecesAreaRect.left - containerRect.left) + Math.random() * (260 - this.pieceSize);
        const randomY = (piecesAreaRect.top - containerRect.top) + Math.random() * (300 - this.pieceSize);
